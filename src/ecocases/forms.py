...
class EcoCaseForm(forms.ModelForm):
    ecocase_title = forms.CharField(widget=TinyMCE(attrs={'rows':1, 'cols':100}), max_length=200)
    ecocase_description = forms.CharField(widget=TinyMCE(attrs={'rows':4, 'cols':100}), max_length=300)
    ecocase_characters = forms.CharField(widget=TinyMCE(attrs={'rows':100, 'cols':500}))
